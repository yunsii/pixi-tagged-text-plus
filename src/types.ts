...
export function isSpriteSource(s: ImageSource): s is SpriteSource {
...
export function isTextureSource(s: ImageSource): s is TextureSource {
  return isImageElement(s) || isBaseTexture(s)
}
...
export function createEmptySegmentToken(): SegmentToken {
  return {
    content: '',
...
export function isWhitespace(s: string): s is WhitespaceToken {
  return s !== ''
    && s.split('').every((char: string): boolean => char.search(/\s/) === 0)
}
export function isNewline(s: string): s is NewlineToken {
  return isWhitespace(s) && s === '\n'
}

export function _isSpriteToken(t: SegmentToken): t is SpriteSegmentToken {
  return t.content instanceof PIXI.Sprite
}
export const isSpriteToken = flatEvery(_isSpriteToken)

export function _isTextToken(t: SegmentToken): t is TextSegmentToken {
  return typeof t.content === 'string'
}
export const isTextToken = flatEvery(_isTextToken)

export function _isWhitespaceToken(t: SegmentToken): t is WhitespaceSegmentToken {
  return t !== undefined && _isTextToken(t) && isWhitespace(t.content)
}
export const isWhitespaceToken = flatEvery(_isWhitespaceToken)

export function _isNewlineToken(t: SegmentToken): t is NewlineSegmentToken {
  return t !== undefined && _isTextToken(t) && isNewline(t.content)
}
export function isNewlineToken(t?: Nested<SegmentToken>): boolean {
  return t === undefined ? false : flatEvery(_isNewlineToken)(t)
}

export const isNotWhitespaceToken = complement(isWhitespaceToken)

export function isEmptyObject<T>(a: T): boolean {
...
export function pixelToNumber(s: string): number {
  return Number(s.trim().slice(0, -2))
}
export const emToNumber = pixelToNumber

export function percentStringToNumber(s: string): number {
  return isPercent(s) ? Number(s.trim().slice(0, -1)) / 100 : Number.NaN
}

export function measurementValueToComponents(input: MeasurementValue): MeasurementComponents {
  if (input === undefined) {
  ...
  if (i !== -1) {
    return {
      value: Number.parseFloat(input.slice(0, i)),
  ...
  if (isAllDigits) {
    const forcedNumberConversion = Number.parseFloat(input)
    if (Number.isNaN(forcedNumberConversion) === false) {
      return { value: Number.parseFloat(input), unit: DEFAULT_MEASUREMENT_UNIT }
    }
  }
  ...
  return { value: Number.NaN, unit: MeasurementUnit.unknown }
}
