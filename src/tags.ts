 ...
 */
export function getTagRegex(tagNamesToMatch: string[] = ['\\w+']): RegExp {
 ...
 */
export function parseAttributes(attributesString = ''): AttributesList {
  if (attributesString === '') {
    return {}
  }

  const attributeMatch = /[a-z][a-z0-9]*=('|")[^'"]*('|")/gi
...
export function createTagMatchData(match: RegExpExecArray): TagMatchData {
  const {
    0: tag,
    1: openTagName,
    2: attributes,
...
export function tagMatchDataToTagWithAttributes(tag: TagMatchData): TagWithAttributes {
  return {
    tagName: tag.tagName,
 ...
 */
export function extractSegments(input: string, tagMatchData: TagMatchData[]): string[] {
...
export function wrapEmoji(input: string): string {
  const emojiRegex = new RegExp(
    `((<|</)[^>]*)?(${getEmojiRegex().source})+`,
    'gmu',
  )

  return input.replace(emojiRegex, (match, tagStart) => {
...
export function replaceSelfClosingTags(input: string): string {
  return input.replace(selfClosingTagSearch, (_, tag, attributes = '') => {
...
export function removeTags(input: string): string {
  return input.replace(getTagRegex(), '')
}
...
export function tagMatchToTagToken(tag: TagMatchData): TagToken {
  return {
    tag: tag.tagName,
    children: [],
...
export function createTokensNew(segments: string[], tags: TagMatchData[], logWarningFunction = defaultLogWarning): (TagToken | TextToken)[] {
...
export function containsEmoji(input: string): boolean {
  return getEmojiRegex().test(input)
}
 ...
 */
export function parseTagsNew(input: string, tagNamesToMatch: string[] = [], shouldWrapEmoji = false, logWarningFunction = defaultLogWarning): CompositeToken<TagToken | TextToken> {
