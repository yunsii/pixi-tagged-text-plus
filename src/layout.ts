 ...
 */
export function updateOffsetForNewLine(offset: Point, largestLineHeight: number, lineSpacing: number): Point {
  return new PIXI.Point(0, offset.y + largestLineHeight + lineSpacing)
}

function rectFromContainer(container: PIXI.Container, offset: Point = { x: 0, y: 0 }): Bounds {
 ...
 */
export function translatePoint<P extends Point>(offset: Point) {
  return (point: P): P => ({
    ...point,
    x: point.x + offset.x,
    y: point.y + offset.y,
 ...
 */
export function translateLine(offset: Point) {
  return (line: Bounds[]): Bounds[] =>
    line.map(translatePoint(offset))
}

export function translateWordPosition(offset: Point) {
  return (word: WordToken): WordToken =>
    word.map((token) =>
      mapProp<Bounds, SegmentToken>('bounds')(translatePoint(offset))(token),
    )
}

export function translateTokenLine(offset: Point) {
  return (line: LineToken): LineToken =>
    line.map(translateWordPosition(offset))
}

export function lineWidth(wordsInLine: Bounds[]): number {
...
function positionWordX(x: number) {
  return (word: WordToken): WordToken => {
...
export function concatBounds(originalBounds: Bounds = { x: Number.NaN, y: Number.NaN, width: Number.NaN, height: Number.NaN }, bounds: Bounds = { x: Number.NaN, y: Number.NaN, width: Number.NaN, height: Number.NaN }): Bounds {
  if (Number.isNaN(originalBounds.x)) {
    return bounds
  }
...
function getCombinedBounds(bounds: Bounds[]): Bounds {
  return bounds.reduce(concatBounds, { x: Number.NaN, y: Number.NaN, width: Number.NaN, height: Number.NaN })
}
...
export function alignLines(align: Align, maxWidth: number, lines: ParagraphToken): ParagraphToken {
  ...
  for (const line of lines) {
    ...
    for (const word of line) {
      ...
      if (Number.isNaN(wordBounds.x)) {
        throw new TypeError('wordBounds not correct')
      }
    }
    if (isLastLine) {
...
function getTallestToken(line: LineToken): SegmentToken {
...
export function verticalAlignInLines(lines: ParagraphToken, lineSpacing: number, overrideValign?: VAlign): ParagraphToken {
...
export function collapseWhitespacesOnEndOfLines(lines: ParagraphToken): ParagraphToken {
  for (const line of lines) {
    const l = line.length
    let i = l
    while (i >= 0) {
...
function layout(tokens: SegmentToken[], maxWidth: number, lineSpacing: number, align: Align, _splitStyle: SplitStyle): ParagraphToken {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    else if (wordInBufferExceedsLineLength() // don't wrap if it's the first word in the line.
      && line.length > 0) {
      addLineToListOfLinesAndMoveCursorToNextLine(token)
    }
  }
...
export function splitAroundWhitespace(s: string): string[] {
  return s
...
export function splitText(s: string, splitStyle: SplitStyle): string[] {
  if (splitStyle === 'words') {
...
export function calculateTokens(styledTokens: StyledTokens, splitStyle: SplitStyle = 'words', scaleIcons = true, adjustFontBaseline?: FontMap): ParagraphToken {
      ...
      (token: StyledToken | TextToken | SpriteToken): SegmentToken[] => {
        ...
        // split into pieces and convert into tokens.
          ...
          const textTokens = textSegments.map((str): SegmentToken => {
            ...
            const scaleWidth = Number.isNaN(sw) || sw < 0 ? 0.0 : sw
            const scaleHeight = Number.isNaN(sh) || sh < 0 ? 0.0 : sh
  ...
  const hasWordWrapWidth = www !== undefined && Number.isNaN(www) === false && www > 0
  const maxWidth
...
export function getBaselineAdjustment(style: TextStyleExtended, fontBaselineMap: FontMap = {}, ascent: number): number {
